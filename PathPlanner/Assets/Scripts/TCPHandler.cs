...
{
    class TCPHandler : MonoBehaviour
    {
        ...
        public static byte[] RecieveData(TcpClient clientSocket, NetworkStream clientStream)
        {
            ...
            try
            {
                while (blnReadMore)
                {
                    ...
                    int smallCounter = 0;
                    while (!clientStream.DataAvailable && smallCounter < Assets.Scripts.ProjectConstants.MaxWaitTime + 1)
                    {
                    ...
                    if (smallCounter > Assets.Scripts.ProjectConstants.MaxWaitTime)
                    {
            ...
            {
                GUI.Box(new Rect(0,0,Screen.width/2,Screen.height/2),ex.Message);
                return null;
            }
        }
